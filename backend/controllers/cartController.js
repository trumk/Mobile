...
exports.addToCart = async (req, res) => {
    ...
    try {
        ...
        const existingItem = cart.items.find(item => item.classType.toString() === classTypeId);
        if (existingItem) {
            return res.status(400).json({ message: 'Class already in cart' });
        }

        cart.items.push({ classType: classTypeId });
        await cart.save();
...
exports.removeFromCart = async (req, res) => {
    ...
    try {
        ...
        cart.items = cart.items.filter(item => item.classType.toString() !== classTypeId);
        await cart.save();
